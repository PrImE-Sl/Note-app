...
createNote = (req, res) => {
    ...
    if (!body) {
        return res.status(400).json({ success: false, error: 'You must provide a note', })
    }
    ...
    note.save()
        ...
        .catch(error => {
            return res.status(400).json({ error, message: 'Note is not created!', })
        })
}
...
updateNote = async (req, res) => {
    ...
    Note.findOne({ _id: req.params.id }, (err, note) => {
        if (err) {
            return res.status(404).json({ err, message: 'Note not found!', })
        }
        note.name = body.name
        note.note = body.note
        note.save().then(() => {
            ...
            .catch(error => {
                return res.status(404).json({ error, message: 'Note not updated!', })
            })
    })
}
